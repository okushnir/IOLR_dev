...
try:
    ...
    print("PyTorch available - using LSTM embeddings")
except ImportError:
    print("PyTorch not available. Using sklearn-based embeddings...")
...
class LSTMEmbedder:
    """PyTorch-based LSTM embedder that learns nonlinear embeddings"""

    def __init__(self, embedding_dim=3, sequence_length=10, hidden_units=32):
        if not PYTORCH_AVAILABLE:
            raise ImportError("PyTorch not available. Use SimplifiedEmbedder instead.")
    ...
    def _build_model(self):
        """Build PyTorch LSTM autoencoder"""

        class LSTMAutoencoder(nn.Module):
            def __init__(self, sequence_length, hidden_units, embedding_dim):
                super().__init__()
                self.sequence_length = sequence_length
                self.embedding_dim = embedding_dim

                # Encoder
                self.encoder_lstm1 = nn.LSTM(1, hidden_units, batch_first=True)
                self.encoder_lstm2 = nn.LSTM(hidden_units, hidden_units // 2, batch_first=True)
                self.encoder_fc = nn.Linear(hidden_units // 2, embedding_dim)

                # Decoder
                self.decoder_fc = nn.Linear(embedding_dim, hidden_units // 2)
                self.decoder_lstm1 = nn.LSTM(hidden_units // 2, hidden_units // 2, batch_first=True)
                self.decoder_lstm2 = nn.LSTM(hidden_units // 2, hidden_units, batch_first=True)
                self.decoder_output = nn.Linear(hidden_units, 1)
            ...
            def forward(self, x):
                ...
                x, (h, c) = self.encoder_lstm2(x)
                embedding = torch.tanh(self.encoder_fc(h[-1]))  # Use last hidden state
        ...
        return LSTMAutoencoder(self.sequence_length, self.hidden_units, self.embedding_dim)

    def fit(self, data, epochs=50, batch_size=32, validation_split=0.2, verbose=0):
        """Fit PyTorch LSTM embedder to time series data"""
        ...
        y_seq = X.clone()  # Autoencoder target is input
        ...
        for epoch in range(epochs):
            ...
            if len(X_val) > 0:
                self.model.eval()
                with torch.no_grad():
                    val_output, _ = self.model(X_val.to(self.device))
                    val_loss = criterion(val_output, y_val.to(self.device))
                self.model.train()
    ...
    def embed(self, data):
        """Generate PyTorch LSTM embeddings for time series"""
        ...
        with torch.no_grad():
            for i in range(len(data_scaled) - self.sequence_length + 1):
                ...
                _, embedding = self.model(seq_tensor)
...
class LSTMCCM:
    ...
    def fit_embedders(self, x_data, y_data, epochs=50, verbose=0):
        ...
        embedder_type = "LSTM" if PYTORCH_AVAILABLE else "PCA"
        print(
...
def compare_embedding_methods():
    ...
    try:
        ...
        print(f"\n{('LSTM' if PYTORCH_AVAILABLE else 'PCA')}-CCM Test Results:")
        print(result)
...
if __name__ == "__main__":
    print("PyTorch-LSTM CCM Causality Analysis Script")
    print("==================================")

    if PYTORCH_AVAILABLE:
        print("✓ PyTorch available - using LSTM embeddings")
    else:
        print("ℹ PyTorch not available - using PCA embeddings")
