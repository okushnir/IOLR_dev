...
def plot_phase_space_reconstruction(data_dict, title, embedding_dim=3, sequence_length=10, save_path=None):
    ...
    plt.suptitle(f'{title}\nLSTM Embedding Space (dim={embedding_dim}, seq_len={sequence_length})', fontsize=16)
    plt.tight_layout()
...
def demonstrate_lstm_embedding_analysis(data_dict, simulation_name, output_dir, skip_sensitivity=True):
    ...
    if skip_sensitivity:
        print(f"\n{'=' * 60}")
        print(f"LSTM EMBEDDING ANALYSIS: {simulation_name} (Sensitivity analysis skipped for speed)")
        print(f"{'=' * 60}")
        print("  Sensitivity analysis disabled for faster execution.")
    ...
    for var1, var2 in test_pairs:
        ...
        results_xy, results_yx, embedding_dims, sequence_lengths = lstm_embedding_sensitivity_analysis(
            data_dict, var1, var2,
            embedding_dims=range(2, 5),  # Reduced range
            sequence_lengths=range(6, 10),  # Reduced range
...
def run_lstm_ccm_simulations():
    ...
    with LogCapture(log_file):
        ...
        print(f"Using NumPy LSTM embeddings for nonlinear time series analysis")
        ...
        visualize_time_series(data1, f"LSTM-CCM Simulation 1: Linear Chain", save_path=ts_plot1)

        # LSTM embedding analysis
        demonstrate_lstm_embedding_analysis(data1, f"LSTM Simulation 1", output_dir)

        print(f"  Analyzing causality with LSTM-CCM...")
        results1 = analyze_all_pairs_lstm(data1, embedding_dim=4, sequence_length=10, epochs=15)
        ...
        visualize_time_series(data2, f"LSTM-CCM Simulation 2: Hub + Outsider", save_path=ts_plot2)

        demonstrate_lstm_embedding_analysis(data2, f"LSTM Simulation 2", output_dir)

        print(f"  Analyzing causality with LSTM-CCM...")
        results2 = analyze_all_pairs_lstm(data2, embedding_dim=4, sequence_length=10, epochs=15)
        ...
        visualize_time_series(data3, f"LSTM-CCM Simulation 3: Complex Network", save_path=ts_plot3)

        demonstrate_lstm_embedding_analysis(data3, f"LSTM Simulation 3", output_dir)

        print(f"  Analyzing causality with LSTM-CCM...")
        results3 = analyze_all_pairs_lstm(data3, embedding_dim=4, sequence_length=10, epochs=15)
        ...
        # Final Summary
        print("\n" + "=" * 80)
        print(f"LSTM-CCM ANALYSIS SUMMARY")
        print("=" * 80)
    ...
    print(f"\nLSTM-CCM simulation completed! Results saved to: {output_dir}")
...
if __name__ == "__main__":
    print("NumPy-LSTM CCM Causality Analysis Script")
    print("==================================")
    print("✓ NumPy LSTM available - using LSTM embeddings")
